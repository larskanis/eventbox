...
Such data races between internal and external scope are avoided through {Eventbox::Sanitizer filters} applied to all inputs and outputs.
...
There is no hard criteria for blocking operations, but if the processing time of an event scope method or block exceeds the limit of 0.5 seconds, a warning is print to STDERR.
...
Although actions reside within the same class they don't share instance variables with event scope.
...
And all data generated by the action should be passed as arguments to event scope methods and should be managed there.

Some data shall just be managed as reference in some scope or it is only passed through a given scope.
...
This is an important difference to [threadsafe collections](https://github.com/ruby-concurrency/concurrent-ruby#thread-safe-value-objects-structures-and-collections) like `Concurrent::Hash`.
They mislead the developer to believe that a module is thread-safe when it's using these classes.
...
Instead it encourages the use of blocking operations and threads for things which should run in parallel, while still keeping the majority of code in safe internal methods written in an event based style.
